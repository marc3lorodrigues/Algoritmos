## Exercícios

### 1. Faça um programa que carregue um vetor de nove elementos numéricos inteiros, calcule e mostre os números primos e suas respectivas posições. </br>
### 2. Faça um programa que carregue dos vetores de dez elementos. Calcule e mostre:
  * A soma de um pelo outro.
  * A multiplicação de um pelo outro.</br>
### 3. Faça um programa que carregue um vetor com oito números inteiros, calcule e mostre dois vetores resultantes. </br>
O primeiro vetor deve conter os números positivos. </br>
O segundo vetor resultante deve conter os números negativos. </br>
Cada vetores vai ter no mácio oito posicoes. Sendo que nem  todas devem obrigatoriamente se utilizada. </br>
[Vetores] (https://github.com/marc3lorodrigues/Algoritmos/blob/main/Vetores.rar)




  
  



